...
remote_ansible_checks () {
    ...
    if [[ ! -f "${inventory_path}" ]]; then
        ...
        echo_red "If it's a Terraform environment, please run: ./infra-automation terraform ${environment} apply"
...
remote_ansible_playbook () {
    ...
    if [[ ! -f "${vault_key_path}" ]]; then
        ...
        echo_red "If it's a new project, run this command to create it: ./infra-automation ansible-vault ${environment} init-key"
